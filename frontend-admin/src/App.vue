...
<template>
  ...
  <main v-else class="shell">
    <aside><div class="brand"><span>H</span> HAITOO</div><p>平台后台</p><nav><button :class="{active:activePage==='overview'}" @click="activePage='overview'">概览</button><button :class="{active:activePage==='companies'}" @click="activePage='companies'">公司管理</button><button :class="{active:activePage==='models'}" @click="activePage='models'">模型管理</button><button :class="{active:activePage==='non-ai-points'}" @click="activePage='non-ai-points'">积分配置</button></nav><div class="operator"><b>{{ user?.name }}</b><small>超级管理员</small><button @click="logout">退出登录</button></div></aside>
    <section class="content"><header><div><p class="eyebrow">PLATFORM ADMIN</p><h1>{{activePage==='overview'?'平台概览':activePage==='companies'?'公司管理':activePage==='models'?'模型管理':'积分配置'}}</h1></div><button class="refresh" @click="loadAdmin">↻ 刷新数据</button></header>

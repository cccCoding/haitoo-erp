    ...
    <section class="content"><header><div><p class="eyebrow">PLATFORM ADMIN</p><h1>{{activePage==='overview'?'平台概览':activePage==='companies'?'公司管理':'模型管理'}}</h1></div><button class="refresh" @click="loadAdmin">↻ 刷新数据</button></header>
      <section class="page"><template v-if="activePage==='overview'"><div class="metrics"><article><span>公司</span><b>{{ overview?.companies ?? 0 }}</b><small>已开通企业</small></article><article><span>店铺</span><b>{{ overview?.shops ?? 0 }}</b><small>已授权店铺</small></article><article><span>平台用户</span><b>{{ overview?.users ?? 0 }}</b><small>含平台管理员</small></article><article><span>进行中任务</span><b>{{ overview?.running_tasks ?? 0 }}</b><small>累计 {{ overview?.tasks ?? 0 }} 个任务</small></article></div><section class="panel queue-health" :class="{'queue-alert':overview?.queue?.alert}"><div class="heading"><div><h2>印花任务队列</h2><p>告警阈值：待处理超过 200 个、最早等待超过 10 分钟，或近 15 分钟失败率超过 10%。</p></div><span class="queue-health-state">{{overview?.queue?.alert?'需要关注':'运行正常'}}</span></div><div class="queue-metrics"><article><span>待处理</span><b>{{overview?.queue?.queued ?? 0}}</b><small>其中重试 {{overview?.queue?.retrying ?? 0}} 个任务</small></article><article><span>运行中</span><b>{{overview?.queue?.running ?? 0}}</b><small>最早等待 {{overview?.queue?.oldest_wait_minutes ?? 0}} 分钟</small></article><article><span>近 1 小时吞吐</span><b>{{overview?.queue?.completed_prints_last_hour ?? 0}}</b><small>{{overview?.queue?.completed_tasks_last_hour ?? 0}} 个任务</small></article><article><span>近 15 分钟失败率</span><b>{{overview?.queue?.failure_rate_15m ?? 0}}%</b><small>近 1 小时最终失败 {{overview?.queue?.failed_tasks_last_hour ?? 0}} 个任务</small></article></div><div class="queue-model-table"><div class="queue-model-head"><span>平台</span><span>模型</span><span>待处理</span><span>运行中</span><span>最终失败</span></div><div v-for="item in overview?.queue?.model_backlog || []" :key="`${item.provider}-${item.model}`" class="queue-model-row"><b>{{item.provider}}</b><code>{{item.model}}</code><span>{{item.queued}}</span><span>{{item.running}}</span><span :class="item.failed?'bad':''">{{item.failed}}</span></div><p v-if="!(overview?.queue?.model_backlog?.length)" class="queue-empty">当前没有积压或失败任务。</p></div></section><section class="panel overview-note"><h2>管理入口</h2><p>在“公司管理”中开通公司并配置妙手账号；在“模型管理”中切换印花贴合模型。</p></section></template>
        ...
        <section v-else class="panel model-panel">
          ...
          <div class="provider-list"><div class="provider-list-head"><span>平台</span><span>模型标识</span><span>单个任务</span><span>密钥状态</span><span>启用状态</span><span>操作</span></div><article v-for="provider in providers" :key="provider.provider" class="provider-row"><div><div class="provider-platform-name"><em v-if="provider.is_default">默认</em><b>{{ provider.display_name }}</b></div><small>{{ provider.credential_env || '未定义环境变量' }}</small></div><code>{{ provider.model }}</code><span>{{ provider.images_per_task }} 张图</span><span class="credential-status" :class="overview?.credential_status?.[provider.provider] ? 'is-ready' : 'is-missing'">{{ overview?.credential_status?.[provider.provider] ? '密钥就绪' : '缺少密钥' }}</span><div class="provider-enabled"><span :class="provider.enabled ? 'ok' : 'bad'">{{ provider.enabled ? '已启用' : '已停用' }}</span></div><div class="provider-row-actions"><button class="secondary" :disabled="!provider.enabled || provider.is_default || saving" @click="setDefault(provider)">设为默认</button><button class="primary compact" @click="openProviderForm(provider)">修改</button></div></article></div>

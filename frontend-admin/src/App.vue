...
const passwordTarget = ref<{ companyId: number; adminId: number; name: string; email: string } | null>(null)
const newPassword = ref(''), confirmPassword = ref(''), passwordError = ref('')
...
async function loadAdmin() {
  ...
  user.value = me.data.user; overview.value = stats.data; providers.value = models.data; companies.value = companyRows.data; queueSettings.value = queue.data
}
...
function openProviderForm(provider: any) { error.value = ''; providerForm.value = { provider: provider.provider, display_name: provider.display_name, model: provider.model, enabled: provider.enabled, is_default: provider.is_default, images_per_task: provider.images_per_task }; showProviderForm.value = true }
async function saveProviderForm() { try { saving.value = providerForm.value.provider; error.value = ''; await api.put(`/admin/ai-providers/${providerForm.value.provider}`, { model: providerForm.value.model, enabled: providerForm.value.enabled, is_default: providerForm.value.is_default, images_per_task: providerForm.value.images_per_task }, { headers: headers.value }); showProviderForm.value = false; await loadAdmin(); showToast('模型配置已保存') } catch (e: any) { error.value = e.response?.data?.detail || '保存失败' } finally { saving.value = '' } }
async function saveQueueSettings() { try { saving.value = 'queue'; error.value = ''; await api.put('/admin/task-queue-settings', queueSettings.value, { headers: headers.value }); await loadAdmin(); showToast('任务间隔已保存') } catch (e: any) { error.value = e.response?.data?.detail || '保存失败' } finally { saving.value = '' } }
...
function openPasswordForm(company: any, admin: any) {
  passwordTarget.value = { companyId: company.id, adminId: admin.id, name: admin.name, email: admin.email }
  newPassword.value = ''; confirmPassword.value = ''; passwordError.value = ''
}
...
async function updateAdminPassword() {
  ...
  if (newPassword.value.length < 8 || newPassword.value.length > 128) { passwordError.value = '密码需为 8–128 个字符'; return }
  if (newPassword.value !== confirmPassword.value) { passwordError.value = '两次输入的密码不一致'; return }
  try {
    saving.value = 'password'; passwordError.value = ''
    await api.put(`/admin/companies/${passwordTarget.value.companyId}/admins/${passwordTarget.value.adminId}/password`, { password: newPassword.value }, { headers: headers.value })
    closePasswordForm()
    showToast('管理员密码已修改，原登录会话已失效')
  } catch (e: any) { passwordError.value = e.response?.data?.detail || '修改密码失败' }
  finally { saving.value = '' }
}
...
<template>
  ...
  <main v-else class="shell">
    ...
    <section class="content"><header><div><p class="eyebrow">PLATFORM ADMIN</p><h1>{{activePage==='overview'?'平台概览':activePage==='companies'?'公司管理':'模型管理'}}</h1></div><button class="refresh" @click="loadAdmin">↻ 刷新数据</button></header>
      <section class="page"><template v-if="activePage==='overview'"><div class="metrics"><article><span>公司</span><b>{{ overview?.companies ?? 0 }}</b><small>已开通企业</small></article><article><span>店铺</span><b>{{ overview?.shops ?? 0 }}</b><small>已授权店铺</small></article><article><span>平台用户</span><b>{{ overview?.users ?? 0 }}</b><small>含平台管理员</small></article><article><span>进行中任务</span><b>{{ overview?.running_tasks ?? 0 }}</b><small>累计 {{ overview?.tasks ?? 0 }} 个任务</small></article></div><section class="panel queue-health" :class="{'queue-alert':overview?.queue?.alert}"><div class="heading"><div><h2>印花任务队列</h2><p>告警阈值：待处理超过 200 个、最早等待超过 10 分钟，或近 15 分钟失败率超过 10%。</p></div><span class="queue-health-state">{{overview?.queue?.alert?'需要关注':'运行正常'}}</span></div><div class="queue-metrics"><article><span>待处理</span><b>{{overview?.queue?.queued ?? 0}}</b><small>其中重试 {{overview?.queue?.retrying ?? 0}} 个任务</small></article><article><span>运行中</span><b>{{overview?.queue?.running ?? 0}}</b><small>最早等待 {{overview?.queue?.oldest_wait_minutes ?? 0}} 分钟</small></article><article><span>近 1 小时吞吐</span><b>{{overview?.queue?.completed_prints_last_hour ?? 0}}</b><small>{{overview?.queue?.completed_tasks_last_hour ?? 0}} 个任务</small></article><article><span>近 15 分钟失败率</span><b>{{overview?.queue?.failure_rate_15m ?? 0}}%</b><small>近 1 小时最终失败 {{overview?.queue?.failed_tasks_last_hour ?? 0}} 个任务</small></article></div><div class="queue-model-table"><div class="queue-model-head"><span>平台</span><span>模型</span><span>待处理</span><span>运行中</span><span>最终失败</span></div><div v-for="item in overview?.queue?.model_backlog || []" :key="`${item.provider}-${item.model}`" class="queue-model-row"><b>{{item.provider}}</b><code>{{item.model}}</code><span>{{item.queued}}</span><span>{{item.running}}</span><span :class="item.failed?'bad':''">{{item.failed}}</span></div><p v-if="!(overview?.queue?.model_backlog?.length)" class="queue-empty">当前没有积压或失败任务。</p></div></section><section class="panel overview-note"><h2>管理入口</h2><p>在“公司管理”中开通公司与管理员账号；在“模型管理”中切换印花贴合模型。</p></section></template>
        <template v-else-if="activePage==='companies'"><section class="panel companies"><div class="heading heading-actions"><div><h2>公司与账号</h2><p>开通公司并管理公司管理员账号；业务配置由各公司管理员自行维护。</p></div><button class="primary compact" @click="showCompanyForm=true">＋ 开通公司</button></div><div class="company-table"><div class="company-head"><span>ID</span><span>公司</span><span>管理员账号</span><span>开通时间</span></div><div v-for="company in companies" :key="company.id" class="company-row"><span>{{company.id}}</span><span><b>{{company.name}}</b></span><div class="company-admin-list"><div v-for="admin in company.admin_users" :key="admin.id" class="company-admin"><b>{{admin.name}}</b><small>{{admin.email}}</small><button class="secondary" type="button" @click="openPasswordForm(company, admin)">修改密码</button></div><small v-if="!company.admin_users.length">未配置管理员</small></div><span>{{new Date(company.created_at).toLocaleDateString()}}</span></div></div></section></template>
        <section v-else class="panel model-panel">
          <div class="heading model-heading"><div><p class="section-kicker">AI CONFIGURATION</p><h2>印花贴合模型</h2><p>每条任务按配置携带一组印花，提交和取结果分别串行处理。</p></div><div class="queue-setting-form"><label>提交间隔（秒）<input v-model.number="queueSettings.submit_interval_seconds" type="number" min="1" max="3600" /></label><label>取结果间隔（秒）<input v-model.number="queueSettings.result_interval_seconds" type="number" min="1" max="3600" /></label><button class="secondary" :disabled="saving==='queue'" @click="saveQueueSettings">{{saving==='queue'?'保存中…':'保存任务间隔'}}</button></div></div>
          <div class="provider-list"><div class="provider-list-head"><span>平台</span><span>模型标识</span><span>单个任务</span><span>启用状态</span><span>操作</span></div><article v-for="provider in providers" :key="provider.provider" class="provider-row"><div><div class="provider-platform-name"><em v-if="provider.is_default">默认</em><b>{{ provider.display_name }}</b></div></div><code>{{ provider.model }}</code><span>{{ provider.images_per_task }} 张图</span><div class="provider-enabled"><span :class="provider.enabled ? 'ok' : 'bad'">{{ provider.enabled ? '已启用' : '已停用' }}</span></div><div class="provider-row-actions"><button class="secondary" :disabled="!provider.enabled || provider.is_default || saving" @click="setDefault(provider)">设为默认</button><button class="primary compact" @click="openProviderForm(provider)">修改</button></div></article></div>
          <p v-if="!overview?.storage_ready" class="notice">需要配置 R2 存储和公网访问地址。未配置时，图片无法上传，AI 任务将失败。</p>
        </section>
  ...
  <div v-if="passwordTarget" class="modal-backdrop" @click.self="saving !== 'password' && closePasswordForm()"><form class="modal" @submit.prevent="updateAdminPassword"><button class="close" type="button" :disabled="saving==='password'" @click="closePasswordForm">×</button><h2>修改管理员密码</h2><p>{{passwordTarget.name}} · {{passwordTarget.email}}</p><p v-if="passwordError" class="error" role="alert">{{passwordError}}</p><label>新密码<input v-model="newPassword" type="password" autocomplete="new-password" minlength="8" maxlength="128" required /><small>8–128 个字符</small></label><label>确认新密码<input v-model="confirmPassword" type="password" autocomplete="new-password" minlength="8" maxlength="128" required /></label><button class="primary" type="submit" :disabled="saving==='password'">{{saving==='password'?'保存中…':'确认修改'}}</button></form></div>
  <div v-if="showProviderForm" class="modal-backdrop" @click.self="showProviderForm=false"><section class="modal provider-modal"><button class="close" @click="showProviderForm=false">×</button><p class="section-kicker">模型配置</p><h2>修改 {{ providerForm.display_name }}</h2><p>调整每条任务提交给服务商的印花数量。</p><label>模型标识<input v-model="providerForm.model" maxlength="120" placeholder="输入服务商提供的模型 ID" /></label><div class="modal-field-grid"><label>单个任务印花图数量<input v-model.number="providerForm.images_per_task" type="number" min="1" max="100" /><small>批量快捷操作会按此数量拆成多条独立任务。</small></label></div><div class="modal-tip"><b>结果映射要求</b><span>只有服务商明确保证输出顺序与输入印花一致时才能设为大于 1；不确定时必须保持 1，避免结果关联错图。</span></div><label class="modal-switch"><input v-model="providerForm.enabled" type="checkbox" /> 启用此模型</label><div class="modal-actions"><button class="secondary" :disabled="saving===providerForm.provider" @click="showProviderForm=false">取消</button><button class="primary" :disabled="saving===providerForm.provider" @click="saveProviderForm">{{saving===providerForm.provider?'保存中…':'保存修改'}}</button></div></section></div>

...
const appliedTaskFilters = ref({ creator_id: null as number | null, status: 'awaiting_selection', created_from: '', created_to: '' })
...
async function changeTaskPage(targetPage: number) { currentTaskPage.value = Math.min(Math.max(1, targetPage), taskPageCount.value); selectedTaskIds.value = []; await refreshTaskList() }
  ...
  <main v-if="token" class="app-shell">
    ...
    <section class="content"><header><div><h1>{{ pageTitle }}</h1></div><div class="context"><button class="member account-button" @click="openMyAccountDialog">{{user?.name}} · {{ user?.role === 'company_admin' ? '管理员' : '运营成员' }}{{user?.user_code ? ` · ${user.user_code}` : ''}}</button><button class="ghost" @click="logout">退出</button></div></header>
      ...
      <section v-else-if="page==='tasks'" class="page">
        ...
        <div v-if="selectedTaskIds.length" class="task-batch-bar"><strong>已选择 {{selectedTaskIds.length}} 个任务</strong><button class="primary" :disabled="batchClaimLoading || batchClaiming" @click="openBatchClaimDialog">批量领取</button></div>

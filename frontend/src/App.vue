...
const showDraftEditDialog = ref(false), editingDraft = ref<any>(null), draftEditTitle = ref(''), draftEditShopId = ref<number | null>(null), draftEditSaving = ref(false), draftEditError = ref('')
...
function openDraftEditDialog(draft: any) { editingDraft.value = draft; draftEditTitle.value = draft.title; draftEditShopId.value = draft.shop_id; draftEditError.value = ''; showDraftEditDialog.value = true }
async function saveDraftEdit() {
  if (!editingDraft.value || !draftEditTitle.value.trim()) { draftEditError.value = '请输入商品标题'; return }
  if (!draftEditShopId.value) { draftEditError.value = '请选择投放店铺'; return }
  try {
...
async function saveMyUserCode() { const userCode=myUserCode.value.trim(); if (userCode && [...userCode].length !== 2) { showToast('用户代码必须恰好为两个字符'); return } try { myAccountSaving.value=true; const {data}=await api.patch('/me',{user_code:userCode || null},{headers:headers.value}); user.value=data; showMyAccountDialog.value=false; showToast('用户代码已保存') } catch(e:any) { showToast(e.response?.data?.detail || '保存用户代码失败') } finally { myAccountSaving.value=false } }
...
<template>
  ...
  <div v-if="showMyAccountDialog" class="modal-backdrop" @click.self="showMyAccountDialog=false"><section class="modal-card"><h2>账号设置</h2><p>设置自己的用户代码；留空可清除。用户代码在本公司内不可重复。</p><label>用户代码 <small>（恰好两个字符）</small><input v-model="myUserCode" maxlength="2" placeholder="例如：CN" /></label><div class="modal-actions"><button class="ghost" @click="showMyAccountDialog=false">取消</button><button class="primary" :disabled="myAccountSaving" @click="saveMyUserCode">{{myAccountSaving ? '保存中…' : '保存'}}</button></div></section></div>
  ...
  <div v-if="showDraftEditDialog" class="modal-backdrop" @click.self="showDraftEditDialog=false"><section class="modal-card material-draft-dialog"><button class="modal-close" @click="showDraftEditDialog=false">×</button><h2>编辑商品草稿</h2><p>修改商品标题或调整投放店铺。</p><div class="draft-edit-preview"><img v-for="url in editingDraft?.image_urls" :key="url" :src="imageUrl(url)" :alt="editingDraft?.title || '商品素材'"/></div><label>商品标题<input v-model="draftEditTitle" maxlength="180" placeholder="请输入商品标题"/></label><label>投放店铺<select v-model="draftEditShopId"><option :value="null">请选择投放店铺</option><option v-for="shop in shops" :key="shop.id" :value="shop.id">{{shop.region}} · {{shop.name}}</option></select></label><p v-if="draftEditError" class="error material-draft-error">{{draftEditError}}</p><div class="modal-actions"><button class="ghost" @click="showDraftEditDialog=false">取消</button><button class="primary" :disabled="draftEditSaving" @click="saveDraftEdit">{{draftEditSaving ? '保存中…' : '保存修改'}}</button></div></section></div>

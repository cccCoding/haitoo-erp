...
const taskPageSize = ref(20), currentTaskPage = ref(1), taskTotal = ref(0), taskActiveCount = ref(0), taskStatusCounts = ref<Record<string, number>>({})
...
async function refreshPendingTaskResults() {
  ...
  try { applyTaskPage((await api.get('/tasks', { headers: headers.value, params: { page: currentTaskPage.value, page_size: taskPageSize.value } })).data) } catch { /* 保留上一次任务状态，等待下次轮询。 */ }
}
onMounted(() => {
...
<template>
  ...
  <div v-if="showPersonalResourcesDialog" class="modal-backdrop" @click.self="showPersonalResourcesDialog=false">
    <section class="modal-card personal-resources-dialog">
      ...
      <h2>管理白底图与创作要求</h2>
      <p>当前模板：{{selectedTemplate?.name}}</p>
      <label v-if="user?.role==='company_admin'" class="resource-owner-picker">员工<select v-model="managedResourceUserId" @change="resetWhiteImageForm();resetPersonalPromptForm();loadManagedTemplateResources()"><option v-for="owner in resourceOwners" :key="owner.id" :value="owner.id">{{owner.name}} · {{owner.email}}</option></select></label>

...
async function refresh() {
  ...
  user.value=me.data.user; company.value=me.data.company; shops.value=s.data; templates.value=t.data; templateGroups.value=g.data; tasks.value=task.data; materialAssets.value=material.data; drafts.value=d.data; aiProviders.value=providers.data
...
async function uploadCreativeAssets() { const urls = Array<string>(creativeAssets.value.length); let cursor = 0; creativeUploadedCount.value = creativeAssets.value.filter(asset => asset.uploadedUrl).length; const worker = async () => { while (cursor < creativeAssets.value.length) { const index = cursor++; const asset = creativeAssets.value[index]; if (asset.uploadedUrl) { urls[index] = asset.uploadedUrl; continue } const {data: signed}=await api.post('/uploads/creative-asset/presign',{content_type:asset.file.type,content_length:asset.file.size},{headers:headers.value}); await axios.put(signed.upload_url, asset.file, {headers:{'Content-Type':asset.file.type}}); asset.uploadedUrl = signed.url; urls[index] = signed.url; creativeUploadedCount.value++ } }; await Promise.all(Array.from({length: Math.min(4, creativeAssets.value.length)}, worker)); return urls }
async function createTask() { if (!creativeAssets.value.length) { creativeAssetError.value = '请先上传至少一张印花图，再开始印花贴合。'; return } if (!creativeRequirement.value.trim()) { showToast('请填写创作要求'); return } if (!selectedTemplateId.value) return; if (!creativeProvider.value || !availableAiProviders.value.some(item => item.provider === creativeProvider.value)) { showToast('暂无可用的 AI 模型，请联系超级管理员在后台启用模型'); return } try { creativeAssetError.value = ''; creativeUploading.value=true; const print_urls=await uploadCreativeAssets(); await api.post('/tasks',{template_id:selectedTemplateId.value,provider:creativeProvider.value,ratio:creativeRatio.value,quality:creativeQuality.value,print_url:print_urls[0],print_urls,creative_requirement:creativeRequirement.value.trim()},{headers:headers.value}); await refresh(); page.value='tasks'; showToast(`已入队 ${print_urls.length} 张印花图`) } catch (e:any) { error.value=e.response?.data?.detail || '创建 AI 任务失败' } finally { creativeUploading.value=false } }
...
function openClaimMaterialsDialog(task: any) { claimingTask.value = task; selectedClaimResultUrls.value = []; showClaimMaterialsDialog.value = true }
...
async function refreshPendingTaskResults() {
  if (!token.value || !tasks.value.some(task => ['queued', 'running'].includes(task.status) || (task.progress?.total && task.progress.completed + task.progress.failed < task.progress.total))) return

...
const selectedMaterialAssetIds = ref<number[]>([]), materialTemplateFilterId = ref<number | null>(null), showMaterialDraftDialog = ref(false), materialDraftTemplateId = ref<number | null>(null), materialDraftTitle = ref(''), materialDraftProductDescription = ref(''), materialDraftSizeChart = ref<File | null>(null), materialDraftSizeChartPreview = ref(''), materialDraftTitleGenerating = ref(false), materialDraftSaving = ref(false)
const showMaterialTemplateDialog = ref(false), materialTemplateId = ref<number | null>(null), materialTemplateSaving = ref(false)
...
const draftPageSize = ref(20), currentDraftPage = ref(1), draftTemplateFilterId = ref<number | null>(null)
const taskPageSize = ref(20), currentTaskPage = ref(1), taskTotal = ref(0), taskActiveCount = ref(0), taskStatusCounts = ref<Record<string, number>>({}), taskCreatorFilterId = ref<number | null>(null), materialCreatorFilterId = ref<number | null>(null)
...
const creativeAssets = ref<CreativeAsset[]>([]), showCreativeAssetsDialog = ref(false), creativeAssetError = ref(''), creativeRequirement = ref(''), creativePromptIndex = ref(''), creativeProvider = ref(''), creativeRatio = ref<'1:1' | '3:4'>('1:1'), creativeQuality = ref<'1K' | '2K'>('1K'), creativeUploading = ref(false), creativeUploadedCount = ref(0)
...
const showTeamResourcesDialog = ref(false), teamResourceTab = ref<'white-images' | 'prompts'>('white-images'), teamResourceUserId = ref<number | null>(null), teamWhiteImages = ref<any[]>([]), teamPrompts = ref<any[]>([]), teamResourcesLoading = ref(false), teamResourceQuery = ref('')
const editingWhiteImage = ref<any>(null), whiteImageForm = ref({ name: '', file: null as File | null })
const editingPersonalPrompt = ref<any>(null), personalPromptForm = ref({ name: '', content: '' })
...
async function refresh() {
  ...
  user.value=me.data.user; company.value=me.data.company; shops.value=s.data; templates.value=t.data; templateGroups.value=g.data; applyTaskPage(task.data); materialAssets.value=material.data; drafts.value=d.data; aiProviders.value=providers.data
...
async function createTask() { if (!creativeAssets.value.length) { creativeAssetError.value = '请先上传至少一张印花图，再开始印花贴合。'; return } if (!selectedWhiteImageId.value) { showToast(personalWhiteImages.value.length ? '请选择产品白底图' : '请先设置该模板的产品白底图'); return } if (!creativeRequirement.value.trim()) { showToast('请填写创作要求'); return } if (!selectedTemplateId.value) return; if (!creativeProvider.value || !availableAiProviders.value.some(item => item.provider === creativeProvider.value)) { showToast('暂无可用的 AI 模型，请联系超级管理员在后台启用模型'); return } try { creativeAssetError.value = ''; creativeUploading.value=true; const print_urls=await uploadCreativeAssets(); const {data}=await api.post('/tasks',{template_id:selectedTemplateId.value,white_image_id:selectedWhiteImageId.value,provider:creativeProvider.value,ratio:creativeRatio.value,quality:creativeQuality.value,print_url:print_urls[0],print_urls,creative_requirement:creativeRequirement.value.trim()},{headers:headers.value}); currentTaskPage.value=1; await refresh(); page.value='tasks'; showToast(`已创建 ${data.total} 条任务，共 ${print_urls.length} 张印花`) } catch (e:any) { error.value=e.response?.data?.detail || '创建 AI 任务失败' } finally { creativeUploading.value=false } }
...
async function loadManagedTemplateResources() {
  if (!selectedTemplateId.value || !managedResourceUserId.value) return
  try {
...
async function openPersonalResourcesDialog(tab: 'white-images' | 'prompts' = 'white-images') {
  if (!selectedTemplateId.value) { showToast('请先选择产品模板'); return }
...
async function loadTeamTemplateResources() {
  if (!selectedTemplateId.value || !teamResourceUserId.value) { teamWhiteImages.value=[]; teamPrompts.value=[]; return }
  try {
...
async function openTeamResourcesDialog() {
  ...
  if (!selectedTemplateId.value) { showToast('请先选择产品模板'); return }
  teamResourceTab.value='white-images'; teamResourceQuery.value=''; teamResourceUserId.value=otherResourceOwners.value[0]?.id || null; teamWhiteImages.value=[]; teamPrompts.value=[]; showTeamResourcesDialog.value=true
...
function editWhiteImage(item:any) { editingWhiteImage.value=item; whiteImageForm.value={name:item.name,file:null} }
...
async function saveWhiteImage() {
  if (!selectedTemplateId.value || !managedResourceUserId.value || !whiteImageForm.value.name.trim()) { showToast('请填写白底图名称'); return }
  if (!editingWhiteImage.value && !whiteImageForm.value.file) { showToast('请上传白底图'); return }
  try {
    personalResourceSaving.value=true
    let image_url: string | undefined
    if (whiteImageForm.value.file) { const form=new FormData(); form.append('file',whiteImageForm.value.file); image_url=(await api.post('/uploads/user-template-white-image',form,{headers:headers.value})).data.url }
    ...
    else await api.post('/user-template-white-images',{template_id:selectedTemplateId.value,user_id:managedResourceUserId.value,name:whiteImageForm.value.name.trim(),image_url},{headers:headers.value})
...
function editPersonalPrompt(item:any) { editingPersonalPrompt.value=item; personalPromptForm.value={name:item.name,content:item.content} }
...
function openMaterialDraftDialog() { materialDraftTemplateId.value = selectedMaterialTemplateId.value; materialDraftTitle.value = ''; materialDraftProductDescription.value = ''; materialDraftSizeChart.value = null; materialDraftSizeChartPreview.value = ''; materialDraftSkuPreviewItems.value = []; if (materialDraftTemplateId.value) onMaterialDraftTemplateChange(); showMaterialDraftDialog.value = true }
...
async function uploadMaterialAssets() {
  if (!pendingMaterialUploadFiles.value.length || !materialUploadTemplateId.value) { showToast('请选择产品模板'); return }
  try {
    materialUploading.value = true
    materialUploadError.value = ''
    const form = new FormData()
    pendingMaterialUploadFiles.value.forEach(file => form.append('files', file))
    form.append('template_id', String(materialUploadTemplateId.value))
...
function openMaterialTemplateDialog() { if (!selectedMaterialAssetIds.value.length) return; materialTemplateId.value = selectedMaterialTemplateId.value; showMaterialTemplateDialog.value = true }
async function setMaterialTemplate() {
  if (!materialTemplateId.value) { showToast('请选择产品模板'); return }
  try {
  ...
  } catch (e: any) { showToast(e.response?.data?.detail || '设置模板失败，请稍后重试') }
  finally { materialTemplateSaving.value = false }
}
...
<template>
  ...
  <div v-if="showPersonalResourcesDialog" class="modal-backdrop" @click.self="showPersonalResourcesDialog=false">
    <section class="modal-card personal-resources-dialog">
      ...
      <h2>管理白底图与创作要求</h2>
      <p>当前模板：{{selectedTemplate?.name}} · 仅管理您自己的内容</p>
  ...
  <div v-if="showTeamResourcesDialog" class="modal-backdrop" @click.self="showTeamResourcesDialog=false">
    <section class="modal-card personal-resources-dialog team-resources-dialog">
      ...
      <h2>查看成员自定义内容</h2>
      <p>当前模板：{{selectedTemplate?.name}} · 此处仅供查看</p>
      ...
      <div v-else-if="otherResourceOwners.length && teamResourceTab==='white-images'" class="resource-list team-resource-list"><article v-for="item in filteredTeamWhiteImages" :key="item.id"><button class="team-white-image" title="查看大图" @click="openImagePreview(item.image_url,item.name)"><img :src="imageUrl(item.image_url)" :alt="item.name"/></button><div><strong>{{item.name}}</strong><small>{{new Date(item.updated_at).toLocaleString()}}</small></div></article><p v-if="!filteredTeamWhiteImages.length" class="empty">没有符合条件的白底图。</p></div>
      <div v-else-if="otherResourceOwners.length" class="resource-list prompt-resource-list team-resource-list"><article v-for="item in filteredTeamPrompts" :key="item.id"><div><strong>{{item.name}}</strong><p>{{item.content}}</p><small>{{new Date(item.updated_at).toLocaleString()}}</small></div></article><p v-if="!filteredTeamPrompts.length" class="empty">没有符合条件的创作要求。</p></div>
  ...
  <div v-if="showMaterialUploadDialog" class="modal-backdrop" @click.self="showMaterialUploadDialog=false"><section class="modal-card material-template-dialog"><button class="modal-close" @click="showMaterialUploadDialog=false">×</button><h2>上传本地素材</h2><p>已选择 {{pendingMaterialUploadFiles.length}} 张图片，请先选择产品模板。</p><label>产品模板<select v-model="materialUploadTemplateId"><option :value="null" disabled>请选择产品模板</option><option v-for="template in templates" :key="template.id" :value="template.id">{{template.name}}</option></select></label><div class="modal-actions"><button class="ghost" @click="showMaterialUploadDialog=false">取消</button><button class="primary" :disabled="materialUploading" @click="uploadMaterialAssets">{{materialUploading ? '上传中…' : '确认上传'}}</button></div></section></div>
  <div v-if="showMaterialTemplateDialog" class="modal-backdrop" @click.self="showMaterialTemplateDialog=false"><section class="modal-card material-template-dialog"><button class="modal-close" @click="showMaterialTemplateDialog=false">×</button><h2>批量设置模板</h2><p>将为已选的 {{selectedMaterialAssetIds.length}} 张素材设置同一产品模板。</p><label>产品模板<select v-model="materialTemplateId"><option :value="null" disabled>请选择产品模板</option><option v-for="template in templates" :key="template.id" :value="template.id">{{template.name}}</option></select></label><div class="modal-actions"><button class="ghost" @click="showMaterialTemplateDialog=false">取消</button><button class="primary" :disabled="materialTemplateSaving" @click="setMaterialTemplate">{{materialTemplateSaving ? '设置中…' : '确认设置'}}</button></div></section></div>
</template>

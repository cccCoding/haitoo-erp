...
const creativeAssets = ref<CreativeAsset[]>([]), showCreativeAssetsDialog = ref(false), creativeAssetError = ref(''), creativeRequirement = ref(''), creativeProvider = ref(''), creativePlacement = ref<'居中印花' | '满版印花'>('满版印花'), creativeRatio = ref<'1:1' | '3:4'>('1:1'), creativeQuality = ref<'1K' | '2K'>('1K')
...
async function createTask() { if (!creativeAssets.value.length) { creativeAssetError.value = '请先上传至少一张印花图，再开始印花贴合。'; return } if (!selectedTemplateId.value || !creativeProvider.value) return; try { creativeAssetError.value = ''; const print_urls=await uploadCreativeAssets(); await api.post('/tasks',{template_id:selectedTemplateId.value,provider:creativeProvider.value,placement:creativePlacement.value,ratio:creativeRatio.value,quality:creativeQuality.value,print_url:print_urls[0],print_urls,creative_requirement:creativeRequirement.value.trim() || null},{headers:headers.value}); await refresh(); page.value='tasks' } catch (e:any) { error.value=e.response?.data?.detail || '创建 AI 任务失败' } }
